...
namespace NewRelish
{
    ...
    [Obsolete]
    public class ScheduleDoctorActivity : AppCompatActivity, NavigationView.IOnNavigationItemSelectedListener
    {
        ...
        protected override async void OnCreate(Bundle savedInstanceState)
        {
            ...
            SetContentView(Resource.Layout.activity_sch_doc);
            Load_State();
            Load_Lga();
            ...
            sp = FindViewById<Spinner>(Resource.Id.sp1);
            ...
            spp = FindViewById<Spinner>(Resource.Id.spL);
        ...
        void Loaditems()
        {
            ...
            Task.Run(async () =>
            {
                ...
                RunOnUiThread(() =>
                {
                    myList = FindViewById<GridView>(Resource.Id.gridViewDoctor);
                    Doctor_Data();
                    myList.Adapter = new DoctorsListAdapter(this, doctors);
        ...
        public void Doctor_Data()/**/
        {
            ...
            if (values != null && values.Count > 0)
            {
                ...
                try
                {
                    ...
                    doctors = (from IList<Object> row in values
                                   //  where row != null
                               select new Doctor()
                               {
                                   DoctorName = row[0].ToString() ?? "",
                                   YearMBBS = row[1].ToString() ?? "",
                                   MDCNFolioNo = row[2].ToString() ?? "",
                                   Institution = row[3].ToString() ?? "",
    ...
    class DoctorsListAdapter : BaseAdapter<Doctor>
    {
        IList<Doctor> Hmodoctor;
        ScheduleDoctorActivity c;
        public DoctorsListAdapter(ScheduleDoctorActivity c, IList<Doctor> doctors)
        {
            this.c = c;
            this.Hmodoctor = doctors;
        }
        public override Doctor this[int position] => Hmodoctor[position];

        public override int Count => Hmodoctor.Count;
        ...
        public override View GetView(int position, View convertView, ViewGroup parent)
        {
            ...
            if (convertView == null)
            {
                view = LayoutInflater.From(parent.Context).Inflate(Resource.Layout.doctors_row, parent, false);
            ...
            view.FindViewById<TextView>(Resource.Id.doctorTextView).Text = Hmodoctor[position].DoctorName.ToString();
            view.FindViewById<TextView>(Resource.Id.MBBSYearTextView).Text = Hmodoctor[position].YearMBBS.ToString();
            view.FindViewById<TextView>(Resource.Id.FolioNoTextView).Text = Hmodoctor[position].MDCNFolioNo.ToString();
            view.FindViewById<TextView>(Resource.Id.institutionTextView).Text = Hmodoctor[position].Institution.ToString();
