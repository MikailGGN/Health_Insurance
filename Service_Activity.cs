...
namespace NewRelish
{
    ...
    public class Service_Activity : AppCompatActivity, NavigationView.IOnNavigationItemSelectedListener
    {
        ListView myList;
        TextView txtService;
        Spinner sp, spp;
        ...
        protected override async void OnCreate(Bundle savedInstanceState)
        {
            ...
            TextView Bartext = FindViewById<TextView>(Resource.Id.textView_homesvc);
            ...
            {
                string s = Intent.GetStringExtra("TheService") ?? "Data not available";
                var p = new Intent(this, typeof(PaymentActivity));
                p.PutExtra("ServiceName", services[e.Position].Name.ToString());
                p.PutExtra("TheService", s);
                StartActivity(p);
        ...
        void LoadListItem()
        {
            ...
            Task.Run(async () =>
            {
                ...
                RunOnUiThread(() =>
                {
                    txtService = FindViewById<TextView>(Resource.Id.txtService);
                    string xy = Intent.GetStringExtra("TheService") ?? "Data not available";
                    if (xy == "Ambulance")
                    {

                        txtService.Text = "Ambulance";
                        myList = FindViewById<ListView>(Resource.Id.listViewserv);
                        Ambulance_Data();
                        myList.Adapter = new ServiceListAdapter(this, services);
                    }
                    else if (xy == "Gym")
                    {
                        txtService.Text = "Gym";
        ...
        public void Pharmacy_Data()/**/
        {
            ...
            if (values != null && values.Count > 0)
            {
                ...
                try
                {
                    ...
                    services = (from IList<Object> row in values
                                    //  where row != null
                                select new HealthServices()
                                {
                                    Name = row[0].ToString() ?? "",
        ...
        public void Ambulance_Data()/**/
        {
            String gspreadsheetId = "11BJ1je9-3vqusvdK2qa-IK-ay8BkCJcyuJYDp28z_j4";
            String gsheet = "Ambulance";
        ...
        public void LGA_Data()/**/
        {
            ...
            if (values != null && values.Count > 0)
            {
                ...
                try
                {
                    ...
                    lgas = (from IList<Object> row in values
                                //  where row != null
                            select new HealthLgas()
                            {
